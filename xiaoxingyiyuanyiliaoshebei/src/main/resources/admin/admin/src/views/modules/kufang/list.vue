        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '库房名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.kufangName" placeholder="库房名称" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '库房类型' : ''">
                        <el-select v-model="searchForm.kufangTypes" placeholder="请选择库房类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in kufangTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('kufang','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoxingyiyuanyiliaoshebei/upload/kufangMuBan.xls"
                        >批量导入库房数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('kufang','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="kufangUploadSuccess"
                                :on-error="kufangUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('kufang','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "kufang.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="kufangUuidNumber"
                                   header-align="center"
                                   label="库房编号">
                        <template slot-scope="scope">
                            {{scope.row.kufangUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('kufang','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('kufang','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('kufang','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              kufangTypesSelectSearch : [],
            form:{
                id : null,
                kufangUuidNumber : null,
                kufangName : null,
                kufangTypes : null,
                kufangAddress : null,
                kufangContent : null,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'kufang',//当前表表名,
                        sumColum : 'kufang_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'kufang_name',//分组字符串字段
                        // types : 'kufang_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.kufangName!= '' && this.searchForm.kufangName!= undefined) {
                    params['kufangName'] = '%' + this.searchForm.kufangName + '%'
                }
         
                if (this.searchForm.kufangTypes!= '' && this.searchForm.kufangTypes!= undefined) {
                    params['kufangTypes'] = this.searchForm.kufangTypes
                }
                                        
                params['kufangDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=kufang_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.kufangTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            kufangUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "kufang/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
